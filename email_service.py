"""
Handles outbound email notifications via SMTP.
"""
...
def send_alert_email(alert: Alert) -> bool:
    ...
    if not alert.recipient_email:
        logger.warning("Alert %s has no recipient email; skipping send.", alert.id)
        return False

    subject = f"[{alert.severity.value.upper()}] Malware Alert: {alert.title}"
